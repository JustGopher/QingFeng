        ...
        html += `
            <div class="mb-4 last:mb-0">
                <div class="flex items-center gap-2 mb-3">
                    <span class="px-2 py-0.5 rounded text-white text-xs font-medium ${code.startsWith('2') ? 'bg-green-500' : code.startsWith('4') ? 'bg-yellow-500' : 'bg-red-500'}">${code}</span>
                    <span class="text-sm" style="color: var(--text-secondary)">${escapeHtml(description)}</span>
                </div>
                <div class="flex gap-2 mb-3">
                    <button onclick="switchSchemaView(this, 'example', '${code}')" class="schema-tab schema-tab-${code} px-3 py-1 text-xs rounded" style="background: var(--primary); color: white">Example</button>
                    <button onclick="switchSchemaView(this, 'model', '${code}')" class="schema-tab schema-tab-${code} px-3 py-1 text-xs rounded border" style="border-color: var(--border)">Model</button>
                </div>
                <div id="schema-example-${code}" class="schema-content schema-content-${code}">
                    <pre class="response-panel rounded p-3 overflow-x-auto text-sm"><code>${schema ? syntaxHighlight(JSON.stringify(generateExample(schema), null, 2)) : '// 无响应体'}</code></pre>
                </div>
                <div id="schema-model-${code}" class="schema-content schema-content-${code} hidden">
                    <div class="rounded p-3 text-sm overflow-x-auto" style="background: var(--bg-tertiary)">
...
function renderSchemaModel(schema, depth = 0, parentKey = '') {
    ...
    if (schema.type === 'object' || schema.properties) {
        ...
        const indent = depth > 0 ? 'ml-3 pl-3 border-l' : '';
        ...
        for (const [key, prop] of Object.entries(properties)) {
            ...
            html += `
                <div class="py-1.5 flex items-start gap-2" style="border-bottom: 1px solid var(--border)">
                    <div class="flex-shrink-0 min-w-[100px]">
                        <span class="text-blue-500 font-mono text-xs">${escapeHtml(key)}</span>
                    ...
                    <div class="flex-1 text-xs" style="color: var(--text-secondary)">
                        ...
                        ${example !== '' ? `<span class="ml-1 opacity-70">示例: ${escapeHtml(String(example))}</span>` : ''}
                    </div>
                </div>
            `;
            
            if (prop.type === 'object' || prop.properties || prop.$ref) {
                html += `<div class="ml-3">${renderSchemaModel(prop, depth + 1, key)}</div>`;
            } else if (prop.type === 'array' && prop.items && (prop.items.type === 'object' || prop.items.properties || prop.items.$ref)) {
                html += `<div class="ml-3 pl-2 border-l" style="border-color: var(--border)">
...
function renderBodyFields(schema, savedValues) {
    ...
    if (!schema || !schema.properties) {
        container.innerHTML = '<p class="text-xs" style="color: var(--text-secondary)">无结构化字段</p>';
        return;
    }
    const properties = schema.properties;
    const required = schema.required || [];
    ...
    html += `<thead><tr style="border-bottom: 1px solid var(--border)">
        <th class="text-left py-1.5 px-2">字段</th>
        <th class="text-left py-1.5 px-2">类型</th>
        <th class="text-left py-1.5 px-2">必填</th>
        <th class="text-left py-1.5 px-2" style="min-width: 150px">值</th>
        <th class="text-left py-1.5 px-2">说明</th>
    </tr></thead><tbody>`;
    ...
    for (const [key, prop] of Object.entries(properties)) {
        ...
        html += `<tr style="border-bottom: 1px solid var(--border)">
            <td class="py-1.5 px-2"><span class="font-mono text-blue-500">${escapeHtml(key)}</span></td>
            <td class="py-1.5 px-2"><span class="px-1 rounded" style="background: var(--bg-tertiary)">${escapeHtml(propType)}</span></td>
            <td class="py-1.5 px-2">${isRequired ? '<span class="text-red-500">*</span>' : '-'}</td>
            <td class="py-1.5 px-2">${renderBodyFieldInput(key, prop, savedValue)}</td>
            <td class="py-1.5 px-2" style="color: var(--text-secondary)">${escapeHtml(description)}</td>
        </tr>`;
    }
...
function renderBodyFieldInput(key, prop, value) {
    ...
    if (prop.enum) {
        ...
        return `<select class="input-field w-full rounded px-2 py-1 text-xs" data-body-field="${escapeHtml(key)}" onchange="onBodyFieldChange()"><option value="">--</option>${options}</select>`;
    }
    if (type === 'boolean') {
        return `<select class="input-field w-full rounded px-2 py-1 text-xs" data-body-field="${escapeHtml(key)}" onchange="onBodyFieldChange()">
            <option value="">--</option><option value="true" ${value === true || value === 'true' ? 'selected' : ''}>true</option><option value="false" ${value === false || value === 'false' ? 'selected' : ''}>false</option></select>`;
    }
    if (type === 'integer' || type === 'number') {
        return `<input type="number" class="input-field w-full rounded px-2 py-1 text-xs" data-body-field="${escapeHtml(key)}" data-type="${type}" value="${escapedValue}" oninput="onBodyFieldChange()">`;
    }
    if (type === 'array' || type === 'object') {
        return `<textarea class="input-field w-full rounded px-2 py-1 text-xs font-mono" rows="1" data-body-field="${escapeHtml(key)}" data-type="${type}" oninput="onBodyFieldChange()">${escapedValue}</textarea>`;
    }
    return `<input type="text" class="input-field w-full rounded px-2 py-1 text-xs" data-body-field="${escapeHtml(key)}" data-type="string" value="${escapedValue}" oninput="onBodyFieldChange()">`;
}
...
function toggleBodyEditMode() {
    ...
    if (bodyEditMode === 'form') {
        ...
        btn.innerHTML = '<i class="fas fa-table mr-1"></i>表单';
        syncBodyToJson();
    } else {
        bodyEditMode = 'form';
        formMode.classList.remove('hidden');
        jsonMode.classList.add('hidden');
        btn.innerHTML = '<i class="fas fa-edit mr-1"></i>JSON';
        syncJsonToFields();
    }
}
